resource "dynatrace_automation_workflow" "Teams_Notification-PAM" {
  # description          = ""
  type                   = "SIMPLE"
  ...
  trigger {
    event {
      ...
      config {
        davis_problem {
          analysis_ready   = true
          custom_filter    = "matchesValue(dt.security_context, \"PAM\") and isNotNull(labels.alerting_profile)"
