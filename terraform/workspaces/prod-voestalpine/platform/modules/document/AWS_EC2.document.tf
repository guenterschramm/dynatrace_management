  ...
  content   = jsonencode({
      ...
      "tiles": {
        ...
        "57": {
          ...
          "description": "",
          "query": "timeseries in = avg(cloud.aws.ec2.NetworkIn.By.InstanceId),\n            by: {InstanceId, aws.account.id, aws.region, dt.smartscape_source.id},\n            filter:{in(aws.account.id, array($AccountId)) AND\n                    in(aws.region, array($Region)) AND\n                    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| sort arrayAvg(in) desc\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "58": {
          ...
          "description": "",
          "query": "timeseries out = avg(cloud.aws.ec2.NetworkOut.By.InstanceId),\n            by: {InstanceId, aws.account.id, aws.region, dt.smartscape_source.id},\n            filter:{in(aws.account.id, array($AccountId)) AND\n                    in(aws.region, array($Region)) AND\n                    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| sort arrayAvg(out) desc\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "66": {
          ...
          "description": "",
          "query": "timeseries x = avg(cloud.aws.ebs.VolumeReadBytes.By.VolumeId),\n        by: {VolumeId, aws.account.id, aws.region, dt.smartscape_source.id},\n            filter:{in(aws.account.id, array($AccountId))\n                    AND in(aws.region, array($Region)) AND\n                    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($VolumeId)[]))) OR\n                      matchesValue($VolumeId, \"ALL\")}\n| fieldsAdd y = arrayAvg(x)\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "68": {
          ...
          "description": "",
          "query": "timeseries x = avg(cloud.aws.ebs.VolumeWriteBytes.By.VolumeId),\n        by: {VolumeId, aws.account.id, aws.region, dt.smartscape_source.id},\n            filter:{in(aws.account.id, array($AccountId))\n                    AND in(aws.region, array($Region)) AND\n                    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($VolumeId)[]))) OR\n                      matchesValue($VolumeId, \"ALL\")}\n| fieldsAdd y = arrayAvg(x)\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "69": {
          ...
          "description": "",
          "query": "timeseries x = avg(cloud.aws.ebs.VolumeReadOps.By.VolumeId),\n        by: {VolumeId, aws.account.id, aws.region, dt.smartscape_source.id},\n            filter:{in(aws.account.id, array($AccountId))\n                    AND in(aws.region, array($Region)) AND\n                    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($VolumeId)[]))) OR\n                      matchesValue($VolumeId, \"ALL\")}\n| fieldsAdd y = arrayAvg(x)\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "70": {
          ...
          "description": "",
          "query": "timeseries x = avg(cloud.aws.ebs.VolumeWriteOps.By.VolumeId),\n        by: {VolumeId, aws.account.id, aws.region, dt.smartscape_source.id},\n            filter:{in(aws.account.id, array($AccountId))\n                    AND in(aws.region, array($Region)) AND\n                    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($VolumeId)[]))) OR\n                      matchesValue($VolumeId, \"ALL\")}\n| fieldsAdd y = arrayAvg(x)\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
