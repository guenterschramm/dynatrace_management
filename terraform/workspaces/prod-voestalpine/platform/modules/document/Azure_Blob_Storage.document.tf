  ...
  content   = jsonencode({
      ...
      "tiles": {
        ...
        "11": {
          ...
          "query": "timeseries cap = count(cloud.azure.microsoft_storage.storageaccounts.blobservices.ContainerCount), by: {azure.resource.name} ,\n            filter:{\n              in(azure.subscription, array($Subscription)) AND\n              in(azure.location, array($Location)) AND\n              in(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| fieldsAdd cap = arrayLast(cap)\n| sort cap desc\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "15": {
          ...
          "description": "Containers are organizers for a set of blobs.",
          "query": "timeseries cnt = count(cloud.azure.microsoft_storage.storageaccounts.blobservices.ContainerCount)\n,\n  filter: {\n    in(azure.subscription, array($Subscription)) AND\n    in(azure.location, array($Location)) AND\n    in(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))\n  }\n| fieldsAdd cnt = arrayLast(cnt)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "21": {
          ...
          "query": "timeseries {ingress = avg(cloud.azure.microsoft_storage.storageaccounts.blobservices.Ingress), egress = avg(cloud.azure.microsoft_storage.storageaccounts.blobservices.Egress)}, by: {azure.resource.name},\nfilter:{\n\tin(azure.subscription, array($Subscription)) AND\n\tin(azure.location, array($Location)) AND\n\tin(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| fieldsAdd ingress = arrayLast(ingress), egress = arrayLast(egress)\n| fields azure.resource.name, ingress,egress\n| sort ingress desc\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "22": {
          ...
          "query": "timeseries tx = count(cloud.azure.microsoft_storage.storageaccounts.blobservices.Transactions), by: {azure.resource.name},\n            filter:{\n              in(azure.subscription, array($Subscription)) AND\n              in(azure.location, array($Location)) AND\n              in(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| fieldsAdd txAvg = arrayLast(tx)\n| fields azure.resource.name, txAvg\n| sort txAvg desc\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "5": {
          ...
          "query": "timeseries egress = avg(cloud.azure.microsoft_storage.storageaccounts.blobservices.Egress), by: {azure.resource.name},\nfilter:{\n\tin(azure.subscription, array($Subscription)) AND\n\tin(azure.location, array($Location)) AND\n\tin(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "6": {
          ...
          "query": "timeseries ingress = avg(cloud.azure.microsoft_storage.storageaccounts.blobservices.Ingress), by: {azure.resource.name},\nfilter:{\n\tin(azure.subscription, array($Subscription)) AND\n\tin(azure.location, array($Location)) AND\n\tin(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "9": {
          ...
          "query": "timeseries lat = avg(cloud.azure.microsoft_storage.storageaccounts.blobservices.SuccessE2ELatency), by: {azure.resource.group},\n            filter:{\n              in(azure.subscription, array($Subscription)) AND\n              in(azure.location, array($Location)) AND\n              in(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
