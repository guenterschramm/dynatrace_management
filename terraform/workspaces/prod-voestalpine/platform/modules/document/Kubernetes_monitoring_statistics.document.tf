  ...
  content   = jsonencode({
      ...
      "tiles": {
        ...
        "10": {
          ...
          "description": "List the availability of ActiveGate workloads in monitored Kubernetes clusters.",
          "query": "// Query the number of all containers in ActiveGate workloads\ntimeseries\n  all = sum(dt.kubernetes.containers, default:1),\n  by: {dt.entity.kubernetes_cluster, dt.entity.cloud_application_namespace, dt.entity.cloud_application},\n  filter:in(dt.entity.cloud_application,classicEntitySelector(\"type(CLOUD_APPLICATION),cloudApplicationLabels(dynatrace.com/component:activegate,component:activegate,app.kubernetes.io/component:activegate)\")) and matchesValue(k8s.cluster.name, $Cluster) and matchesValue(entityAttr(dt.entity.kubernetes_cluster, \"activeGateId\"), $ActiveGateId),\n  shift: -2m\n| join  [\n  // Query the number of all running containers in ActiveGate workloads\n  timeseries\n  running = sum(dt.kubernetes.containers, default:0),\n  by: {dt.entity.kubernetes_cluster,dt.entity.cloud_application_namespace,dt.entity.cloud_application},\n  filter:in(dt.entity.cloud_application,classicEntitySelector(\"type(CLOUD_APPLICATION),cloudApplicationLabels(dynatrace.com/component:activegate,component:activegate,app.kubernetes.io/component:activegate)\")) \n    and (container_state == \"running\") and matchesValue(k8s.cluster.name, $Cluster) and matchesValue(entityAttr(dt.entity.kubernetes_cluster, \"activeGateId\"), $ActiveGateId),\n  shift: -2m\n// Join the results of ActiveGate containers with the number of running ActiveGate containers\n], kind:leftOuter, on: {dt.entity.cloud_application, dt.entity.cloud_application_namespace, dt.entity.kubernetes_cluster}\n// Calculate the availability in %: running / all * 100, default 0%\n| fieldsAdd availabilityA = arrayAvg(iCollectArray(right.running[] * 100 / all[]))\n| fieldsAdd availability = coalesce(availabilityA, 0.0)\n// Fetch entity names for cluster, namespace and workload\n| fields entityName(dt.entity.kubernetes_cluster), entityName(dt.entity.cloud_application_namespace), entityName(dt.entity.cloud_application), entityAttr(dt.entity.kubernetes_cluster, \"activeGateId\"), availability\n// Add ActiveGateUrl as link to classic ActiveGate deployment status page\n| fieldsAdd activeGateURL = concat(\"/ui/apps/dynatrace.classic/ui/deploymentstatus/activegates?filter=ID_HEX%3A\", dt.entity.kubernetes_cluster.activeGateId)\n| fieldsAdd activeGateLink = concat(\"[\",dt.entity.kubernetes_cluster.activeGateId,\"](\",activeGateURL,\")\")\n// Rename fields for better visualization\n| fieldsRename `Availability (%)` = availability, `Kubernetes cluster` = dt.entity.kubernetes_cluster.name, `Kubernetes namespace` = dt.entity.cloud_application_namespace.name, `Kubernetes workload` = dt.entity.cloud_application.name, `ActiveGate ID` = activeGateLink\n// Sort by the lowest availability\n| sort `Availability (%)` asc",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
          ...
          "title": "Availability of in-cluster ActiveGates",
          "type": "data",
          "visualization": "table",
          "visualizationSettings": {
            "autoSelectVisualization": false,
            ...
            "customLinks": [
              {
                ...
                "urlPattern": "/ui/intent/dynatrace.settings/open_settings#%7B%22settings.entityId%22%3A%22{{id_classic}}%22%2C%22settings.itemId%22%3A%22kubernetes-cluster-connection%22%7D"
              }
            ],
            "version": 1
          },
      ...
      "variables": [
        {
          ...
          "editable": true,
          "input": "// filter timeseries by the selected cluster\nfetch dt.entity.kubernetes_cluster\n| fields entity.name\n| sort entity.name asc\n// if not clusters, add * placeholder to avoid errors\n| summarize entries = collectArray(entity.name)\n| fields records = if(arraySize(entries) \u003e 0, entries, else: { \"no-clusters-monitored\" })\n| expand records",
          "key": "Cluster",
          "multiple": true,
          "type": "query",
          "version": 2,
