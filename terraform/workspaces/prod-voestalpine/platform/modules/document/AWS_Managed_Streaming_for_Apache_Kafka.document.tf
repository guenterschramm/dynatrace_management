  ...
  content   = jsonencode({
      ...
      "tiles": {
        ...
        "2": {
          ...
          "description": "",
          "query": "timeseries x = avg(cloud.aws.kafka.BytesInPerSec.By.Broker_ID.Cluster_Name),\n            by: {`Cluster Name`, aws.account.id, aws.region, dt.smartscape_source.id},\n            filter:{in(aws.account.id, array($AccountId)) AND\n                    in(aws.region, array($Region)) AND\n                    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| sort arrayAvg(x) desc\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "3": {
          ...
          "description": "",
          "query": "timeseries x = avg(cloud.aws.kafka.BytesOutPerSec.By.Broker_ID.Cluster_Name),\n            by: {`Cluster Name`, aws.account.id, aws.region, dt.smartscape_source.id},\n            filter:{in(aws.account.id, array($AccountId)) AND\n                    in(aws.region, array($Region)) AND\n                    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| sort arrayAvg(x) desc\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
