resource "dynatrace_document" "Store_BizEvent_in_Azure_storage" {
  name      = "Store BizEvent in Azure storage"
  ...
  content   =<<-EOT
    metadata:
      ...
      trackingId: store-bizevent-in-azure-blob
    workflow:
      title: Store BizEvent in Azure Blob Container
      description: Stores BizEvent data from Dynatrace in an Azure Blob Storage container.
      schemaVersion: 4
      trigger: {}
      result: null
      type: STANDARD
      ...
      guide: |-
        ...
        - Adjust the blob name/path in the upload step to organize data by date or event type.
      ...
      tasks:
        ...
        blob_storage_upload_1:
          ...
          input:
            url: '{{task("blob_storage_create_container_1").input.url}}/{{task("blob_storage_create_container_1").input.containerName}}/logs1.txt'
            data: '{{ result("fetch_bizevents").records[0] | string }}'
            connection: ''
        ...
        blob_storage_create_container_1:
          ...
          input:
            url: https://azureworkflowdelete.blob.core.windows.net
            connection: ''
            containerName: workflow-container-test
          ...
          predecessors: []
    EOT
  custom_id = "dynatrace.microsoft.azure.connector.store-bizevent-in-azure-blob-container"
  # private = false
}
