resource "dynatrace_document" "Email_Notifications_for_aggregated_error_logs_via_Microsoft" {
  name      = "Email Notifications for aggregated error logs via Microsoft 365"
  type      = "workflow-template"
  content   =<<-EOT
    metadata:
      version: '1'
      trackingId: send-email-log-summary-via-m365
      dependencies:
        apps:
      ...
      - type: connection
        ...
        targets:
        - tasks.send_log_summary.connection
    workflow:
      title: Send Email with aggregated log summary via Microsoft 365
      description: ''
      schemaVersion: 3
      trigger: {}
      result: null
      ...
      guide: |
        # Send email notifications for error logs with distributed trace links via Microsoft 365
    
        This workflow detects error logs, groups them by content pattern, and sends an email notification via Microsoft 365 with clickable links to distributed traces for quick troubleshooting.
      ...
      tasks:
        ...
        acquire_tenant_information:
          ...
          predecessors: []
    EOT
  custom_id = "dynatrace.microsoft365.connector.send-email-with-aggregated-log-summary-via-m365"
  # private = false
}
