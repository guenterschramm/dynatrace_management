param(
    ...
    [string]$TargetFolder = "C:\Tools\Terraform\tf-dynatrace\voestalpine\environment",
    [string[]]$Resources = @()
)
...
try {
    ...
    if (-not $vars.ContainsKey("tenant_prod_env_url")) {
        throw "Variable 'tenant_prod_env_url' fehlt in $TfvarsPath"
    }
    ...
    $env:DYNATRACE_ENV_URL = $vars["tenant_prod_env_url"]
    $env:DYNATRACE_API_TOKEN = $vars["tenant_prod_api_token"]
