resource "dynatrace_json_dashboard" "xECM_P_Corporate-Web_Application_Performance_owned_by_Dietmar_Wallner_voestalpine_com" {
  contents = jsonencode({
      "dashboardMetadata": {
        "dashboardFilter": {
          "managementZone": {
        ...
        "name": "xECM P (Corporate) - Web Application Performance",
        "owner": "Dietmar.Wallner@voestalpine.com"
      },
      "tiles": [
        {
          "assignedEntities": [
            "APPLICATION-4342A6B443691537"
          ],
          "bounds": {
            "height": 304,
            "left": 570,
        ...
        },
        {
          ...
          "customName": "Data explorer results",
          "metricExpressions": [
            "resolution=null\u0026(builtin:apps.web.domInteractive.load.browser:filter(and(or(in(\"dt.entity.application\",entitySelector(\"type(application),entityName.equals(~\"xECM Corporate~\")\"))))):splitBy():sort(value(auto,descending)):limit(20)):limit(100):names"
          ],
        ...
        },
        {
          ...
          "metricExpressions": [
            "resolution=Inf\u0026(builtin:apps.web.activeSessions:filter(and(or(in(\"dt.entity.application\",entitySelector(\"type(application),entityName.equals(~\"xECM Corporate~\")\"))))):splitBy():sort(value(auto,descending)):limit(20)):limit(100):names",
            "resolution=null\u0026(builtin:apps.web.activeSessions:filter(and(or(in(\"dt.entity.application\",entitySelector(\"type(application),entityName.equals(~\"xECM Corporate~\")\"))))):splitBy():sort(value(auto,descending)):limit(20))"
          ],
  ...
  link_id  = "${dynatrace_json_dashboard_base.xECM_P_Corporate-Web_Application_Performance_owned_by_Dietmar_Wallner_voestalpine_com.id}"
}


resource "dynatrace_dashboard_sharing" "xECM_P_Corporate-Web_Application_Performance_owned_by_Dietmar_Wallner_voestalpine_com" {
  # enabled    = false
  dashboard_id = "${dynatrace_json_dashboard_base.xECM_P_Corporate-Web_Application_Performance_owned_by_Dietmar_Wallner_voestalpine_com.id}"
  # preset     = false
  permissions {
    permission {
      type  = "ALL"
...
resource "dynatrace_json_dashboard_base" "xECM_P_Corporate-Web_Application_Performance_owned_by_Dietmar_Wallner_voestalpine_com" {
  # contents = ""
}
