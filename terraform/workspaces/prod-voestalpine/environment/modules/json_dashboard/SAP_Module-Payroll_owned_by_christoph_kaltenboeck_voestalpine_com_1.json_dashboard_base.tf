resource "dynatrace_json_dashboard" "SAP_Module-Payroll_owned_by_christoph_kaltenboeck_voestalpine_com_1" {
  contents = jsonencode({
      "dashboardMetadata": {
      ...
      "tiles": [
        ...
        },
        {
          ...
          "name": "DTAQL",
          "query": "select useraction.name as [Top T-Codes], usersession.userActionCount as [Action Count] FROM useraction where useraction.name STARTSWITH \"Payroll\" GROUP BY useraction.name, usersession.userActionCount order by usersession.userActionCount DESC",
          "tileType": "DTAQL",
          "timeFrameShift": "-2h",
          "type": "TABLE"
        },
        ...
        },
        {
          ...
          "customName": "Average Response Time",
          "name": "DTAQL",
          "query": "select  startTime as [Time], useraction.duration as [Average Response Time], usersession.userActionCount FROM useraction where useraction.name STARTSWITH \"Payroll\"",
          "tileType": "DTAQL",
          "type": "LINE_CHART",
          "visualizationConfig": {
  ...
  link_id  = "${dynatrace_json_dashboard_base.SAP_Module-Payroll_owned_by_christoph_kaltenboeck_voestalpine_com_1.id}"
}


resource "dynatrace_dashboard_sharing" "SAP_Module-Payroll_owned_by_christoph_kaltenboeck_voestalpine_com_1" {
  enabled      = true
  dashboard_id = "${dynatrace_json_dashboard_base.SAP_Module-Payroll_owned_by_christoph_kaltenboeck_voestalpine_com_1.id}"
  # preset     = false
  permissions {
    permission {
      type  = "ALL"
...
resource "dynatrace_json_dashboard_base" "SAP_Module-Payroll_owned_by_christoph_kaltenboeck_voestalpine_com_1" {
  # contents = ""
}
