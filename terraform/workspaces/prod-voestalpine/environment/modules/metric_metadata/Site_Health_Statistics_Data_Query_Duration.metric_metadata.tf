resource "dynatrace_metric_metadata" "Site_Health_Statistics_Data_Query_Duration" {
  description  = "Duration of the paged query sites.get_site, used to collect site health metrics."
  display_name = "Site Health Statistics Data Query Duration"
  metric_id    = "metric-sfm.cisco.cc.sites.get_site_health.duration"
  unit         = "Seconds"
  dimensions {
    dimension {
