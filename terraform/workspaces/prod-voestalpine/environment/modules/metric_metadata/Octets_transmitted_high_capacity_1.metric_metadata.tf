resource "dynatrace_metric_metadata" "Octets_transmitted_high_capacity_1" {
  description        = "The total number of octets transmitted out of the interface including framing characters. This metric uses 64 bit counter and should be used for highspeed devices."
  display_name       = "Octets transmitted (high capacity)"
  metric_id          = "metric-com.dynatrace.extension.snmp-generic-cisco-device.if.hc.out.octets.count"
  source_entity_type = "snmp:com_dynatrace_extension_snmp_generic_cisco_network_interface"
  unit               = "Count"
}
