resource "dynatrace_web_application" "PAM-Privileged_Access_Management" {
  ...
  meta_data_capture_settings {
    capture {
      name              = "VisitTag1"
      type              = "CSS_SELECTOR"
      capturing_name    = "#apps-container > pv-user-menu > div > div.user-menu__user__wrapper > button > span.user-menu__user__name.ng-star-inserted"
  ...
  user_action_naming_settings {
      ...
      rule {
        ...
        conditions {
          condition {
            ...
            operand2 = "/PasswordVault/v10/Accounts"
            operator = "CONTAINS"
          }
        }
      }
      rule {
        template            = "PSM_Gateway (RDP in Browser)"
        # use_or_conditions = false
        conditions {
          condition {
            operand1 = "{pageUrl (default)}"
            operand2 = "https://pamhtmlgw.voestalpine.net:8443/guac/direct"
            operator = "CONTAINS"
          }
        }
      }
    ...
    xhr_action_naming_rules {
      rule {
        ...
        conditions {
          condition {
            ...
            operand2 = "/PasswordVault/api/auth/saml/logon"
            operator = "CONTAINS"
          }
        }
      }
      rule {
        template            = "Copy Password (de)"
        # use_or_conditions = false
        conditions {
          condition {
            operand1 = "{xhrUrl (default)}"
            operand2 = "/Prerequisites"
            operator = "ENDS_WITH"
          }
          condition {
      ...
      rule {
        template            = "View Account Details"
        # use_or_conditions = false
        conditions {
          condition {
            operand1 = "{xhrUrl (default)}"
            operand2 = "/PasswordVault/api/ExtendedAccounts/"
