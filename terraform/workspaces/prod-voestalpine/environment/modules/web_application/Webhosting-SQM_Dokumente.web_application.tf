resource "dynatrace_web_application" "Webhosting-SQM_Dokumente" {
  name                                 = "Webhosting - SQM Dokumente"
  type                                 = "AUTO_INJECTED"
  ...
  user_action_naming_settings {
      ...
      rule {
        ...
        conditions {
          condition {
            ...
            operand2 = "/sqmdocs.voestalpine.net/sqmdocuments/myTasks"
            operator = "CONTAINS"
          }
        }
      }
    ...
    xhr_action_naming_rules {
      rule {
        template            = "Empfang bestätigt"
        # use_or_conditions = false
        conditions {
          condition {
            operand1 = "{pageUrl (default)}"
            operand2 = "/sqmdocuments/myTasks"
            operator = "CONTAINS"
          }
          condition {
            operand1 = "{elementIdentifier (default)}"
            operand2 = "Empfang bestätigen"
            operator = "CONTAINS"
          }
          condition {
      ...
      rule {
        template            = "DoppelClick Bestätigung"
        # use_or_conditions = false
        conditions {
          condition {
            operand1 = "{xhrUrl (default)}"
            operand2 = "sqmdocuments/api/action/ConfirmReception/"
            operator = "CONTAINS"
          }
        }
      }
