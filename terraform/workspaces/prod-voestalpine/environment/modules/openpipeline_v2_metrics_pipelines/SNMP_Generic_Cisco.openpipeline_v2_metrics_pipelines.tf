resource "dynatrace_openpipeline_v2_metrics_pipelines" "SNMP_Generic_Cisco" {
  custom_id    = "extension:generic-cisco-device"
  ...
  processing {
    processors {
      processor {
        ...
        dql {
          script =<<-EOT
            fieldsAdd mac=array(chassis.mac), troubleshooting.upsert_source="extension:generic-cisco-device|metric:sysuptime", vendor="Cisco"
          EOT
        }
      }
      processor {
        ...
        dql {
          script = "fieldsAdd mac=if(isNotNull(mac.address), array(mac.address)), troubleshooting.upsert_source=\"extension:generic-cisco-device|metric:if.status\""
        }
      }
    }
  }
  smartscape_node_extraction {
    processors {
        ...
        smartscape_node {
          ...
          fields_to_extract {
            ...
            smartscape_field_extraction_entry {
              ...
              referenced_field_name = "software_revision"
            }
            smartscape_field_extraction_entry {
              field_name            = "services"
              referenced_field_name = "sys.services"
            }
            smartscape_field_extraction_entry {
      ...
      processor {
        description = "Network device upsert from cisco component state metric"
        type        = "smartscapeNode"
        enabled     = true
        id          = "network-device-u-cisco-stack"
        matcher     = "metric.key == \"com.dynatrace.extension.snmp-generic-cisco-device.component.state\" and isNotNull(sys.name)"
        smartscape_node {
          extract_node       = true
          node_id_field_name = "dt.smartscape.ext_network_device"
          node_type          = "EXT_NETWORK_DEVICE"
          fields_to_extract {
            smartscape_field_extraction_entry {
              field_name            = "stack_serial_number"
              referenced_field_name = "component.serial_number"
            }
            smartscape_field_extraction_entry {
              field_name            = "stack_model_name"
              referenced_field_name = "component.model_name"
            }
          }
          id_components {
            id_component {
      ...
      processor {
        ...
        matcher     =<<-EOT
          (matchesValue(metric.key, "com.dynatrace.extension.network_device.if.*") or matchesValue(metric.key, "com.dynatrace.extension.snmp-generic-cisco-device.if.*")) and isNotNull(if.name)
        EOT
        smartscape_node {
          extract_node       = false
          node_id_field_name = "dt.smartscape.ext_network_interface"
      ...
      processor {
        ...
        matcher     =<<-EOT
          (matchesValue(metric.key, "com.dynatrace.extension.network_device.if.*") or matchesValue(metric.key, "com.dynatrace.extension.snmp-generic-cisco-device.if.*")) and isNotNull(interface.name)
        EOT
        smartscape_node {
          extract_node       = false
          node_id_field_name = "dt.smartscape.ext_network_interface"
