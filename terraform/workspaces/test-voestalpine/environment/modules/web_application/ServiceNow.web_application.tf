resource "dynatrace_web_application" "ServiceNow" {
  name                                 = "ServiceNow"
  ...
  meta_data_capture_settings {
    capture {
      name              = "VisitTag0"
      type              = "CSS_SELECTOR"
      capturing_name    = "#profile-dropdown > span.visible-lg-inline.ng-binding"
  ...
  user_action_naming_settings {
    ...
    load_action_naming_rules {
      rule {
        template            = "Service Portal: {pageTitle (default)}"
        # use_or_conditions = false
        conditions {
          condition {
            operand1 = "{pageUrl (default)}"
            operand2 = "https://dev309637.service-now.com/sp"
            operator = "CONTAINS"
          }
        }
      }
      rule {
        template            = "Internal: {pageTitle (default)}"
        # use_or_conditions = false
        conditions {
          condition {
            operand1 = "{pageUrl (default)}"
    ...
    xhr_action_naming_rules {
      rule {
        template            = "Service Portal: {elementIdentifier (default)}"
        # use_or_conditions = false
        conditions {
          condition {
            operand1 = "{pageUrl (default)}"
