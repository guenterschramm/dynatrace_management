  ...
  content   = jsonencode({
      ...
      "tiles": {
        ...
        "11": {
          ...
          "query": "timeseries {inFlows = sum(cloud.azure.microsoft_compute.virtualmachines.InboundFlows), outFlows = sum(cloud.azure.microsoft_compute.virtualmachines.OutboundFlows)},\nby:{azure.resource.name, azure.subscription, azure.location, azure.resource.group, dt.smartscape_source.id},\nfilter:{\n\tin(azure.subscription, array($Subscription)) AND\n\tin(azure.location, array($Location)) AND\n\tin(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| fieldsAdd inFlowsLast = arrayLast(inFlows)\n| fieldsAdd outFlowsLast = arrayLast(outFlows)\n| sort inFlowsLast + outFlowsLast desc\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "12": {
          ...
          "title": "CPU utilization rate for top $Limit instances with highest usage",
          "type": "data",
          "visualization": "categoricalBarChart",
          "visualizationSettings": {
            "autoSelectVisualization": false,
        ...
        "13": {
          ...
          "title": "Virtual machines per VM size",
          "type": "data",
          "visualization": "categoricalBarChart",
          "visualizationSettings": {
            "autoSelectVisualization": false,
        ...
        "15": {
          ...
          "title": "Virtual machines per OS type",
          "type": "data",
        ...
        "16": {
          ...
          "title": "Total Virtual Machine instances",
          "type": "data",
        ...
        "2": {
          ...
          "query": "timeseries NetworkInput = sum(cloud.azure.microsoft_compute.virtualmachines.NetworkInTotal),\nby:{azure.resource.name, azure.subscription, azure.location, azure.resource.group, dt.smartscape_source.id},\nfilter:{\n\tin(azure.subscription, array($Subscription)) AND\n\tin(azure.location, array($Location)) AND\n\tin(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| sort arraySum(NetworkInput) desc\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "29": {
          ...
          "description": "Aggregated host signal indicating whether the VM scale set instance is running and available. 1 = available, 0 = unavailable (stopped or rebooting), null = state unknown (e.g., stopped via Azure portal/CLI/PowerShell).",
          "query": "timeseries m1=avg(cloud.azure.microsoft_compute.virtualmachinescalesets.VmAvailabilityMetric),\nby:{azure.resource.name, azure.subscription, azure.location, azure.resource.group, dt.smartscape_source.id},\nfilter:{\n\tin(azure.subscription, array($Subscription)) AND\n\tin(azure.location, array($Location)) AND\n\tin(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($ScaleSetSmartscapeId)[])))}\n| sort arrayAvg(m1) desc\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "31": {
          ...
          "query": "timeseries {read = sum(cloud.azure.microsoft_compute.virtualmachinescalesets.DiskReadBytes),\n          write = sum(cloud.azure.microsoft_compute.virtualmachinescalesets.DiskWriteBytes)},\n          by:{azure.resource.name, azure.subscription, azure.location, azure.resource.group, dt.smartscape_source.id},\n          filter:{\n\tin(azure.subscription, array($Subscription)) AND\n\tin(azure.location, array($Location)) AND\n\tin(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($ScaleSetSmartscapeId)[])))}\n| sort arraySum(read) + arraySum(write) desc\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "37": {
          ...
          "description": "Aggregated host signal indicating whether the virtual machine is running and available. 1 = available, 0 = unavailable (stopped or rebooting), null = state unknown (e.g., stopped via Azure portal/CLI/PowerShell).",
          "query": "timeseries m1=avg(cloud.azure.microsoft_compute.virtualmachines.VmAvailabilityMetric),\nby:{azure.resource.name, azure.subscription, azure.location, azure.resource.group, dt.smartscape_source.id},\nfilter:{\n\tin(azure.subscription, array($Subscription)) AND\n\tin(azure.location, array($Location)) AND\n\tin(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| sort arrayAvg(m1) desc\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "6": {
          ...
          "query": "timeseries availMemB = avg(cloud.azure.microsoft_compute.virtualmachines.AvailableMemoryBytes),\nby:{azure.resource.name, azure.subscription, azure.location, azure.resource.group, dt.smartscape_source.id},\nfilter:{\n\tin(azure.subscription, array($Subscription)) AND\n\tin(azure.location, array($Location)) AND\n\tin(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| sort arrayAvg(availMemB) desc\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
