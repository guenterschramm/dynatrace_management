  ...
  content   = jsonencode({
      ...
      "tiles": {
        ...
        "46": {
          ...
          "query": "timeseries throttled = sum(cloud.azure.microsoft_storage.storageaccounts.Transactions),\n  by: {responsetype},\n  filter: {\n    in(azure.subscription, array($Subscription)) AND\n    in(azure.location, array($Location)) AND\n    in(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))\n  }\n| filter responsetype == \"ClientThrottlingError\"\n| fieldsAdd Throttled = arraySum(throttled)\n| summarize Throttled = sum(Throttled)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "52": {
          ...
          "query": "timeseries used = avg(cloud.azure.microsoft_storage.storageaccounts.UsedCapacity)\n,\n  filter: {\n    in(azure.subscription, array($Subscription)) AND\n    in(azure.location, array($Location)) AND\n    in(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))\n  }\n| fieldsAdd UsedCapacity = arrayLast(used)\n| summarize UsedCapacity = sum(UsedCapacity)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "68": {
          ...
          "query": "timeseries egress = sum(cloud.azure.microsoft_storage.storageaccounts.queueservices.Egress), by: {azure.resource.name},\n            filter:{\n              in(azure.subscription, array($Subscription)) AND\n              in(azure.location, array($Location)) AND\n              in(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "69": {
          ...
          "query": "timeseries ingress = sum(cloud.azure.microsoft_storage.storageaccounts.queueservices.Ingress), by: {azure.resource.name},\n            filter:{\n              in(azure.subscription, array($Subscription)) AND\n              in(azure.location, array($Location)) AND\n              in(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "86": {
          ...
          "title": "File service errors by resource over time",
          "type": "data",
        ...
        "90": {
          ...
          "query": "timeseries avabil = avg(cloud.azure.microsoft_storage.storageaccounts.tableservices.Availability)\n,\n  filter: {\n    in(azure.subscription, array($Subscription)) AND\n    in(azure.location, array($Location)) AND\n    in(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))\n  }\n| fieldsAdd Availability = arrayAvg(avabil)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
          ...
          "title": "Table Storage availability",
          "type": "data",
          "visualization": "singleValue",
          "visualizationSettings": {
            "autoSelectVisualization": false,
        ...
        "93": {
          ...
          "query": "timeseries egress = sum(cloud.azure.microsoft_storage.storageaccounts.tableservices.Egress), by: {azure.resource.name},\n            filter:{\n              in(azure.subscription, array($Subscription)) AND\n              in(azure.location, array($Location)) AND\n              in(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "94": {
          ...
          "query": "timeseries ingress = sum(cloud.azure.microsoft_storage.storageaccounts.tableservices.Ingress), by: {azure.resource.name},\n            filter:{\n              in(azure.subscription, array($Subscription)) AND\n              in(azure.location, array($Location)) AND\n              in(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "95": {
          ...
          "query": "timeseries resp = sum(cloud.azure.microsoft_storage.storageaccounts.tableservices.Transactions), by: {responsetype},\n            filter:{\n              in(azure.subscription, array($Subscription)) AND\n              in(azure.location, array($Location)) AND\n              in(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| fieldsAdd response = arraySum(resp)\n| sort response desc",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "96": {
          ...
          "query": "timeseries sum(cloud.azure.microsoft_storage.storageaccounts.tableservices.Transactions), by: {azure.resource.name, responsetype},\n            filter:{\n              in(azure.subscription, array($Subscription)) AND\n              in(azure.location, array($Location)) AND\n              in(azure.resource.group, array($ResourceGroup)) AND\n    in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| filter responsetype != \"Success\"\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
          ...
          "title": "Table service errors by resource over time",
          "type": "data",
          "visualization": "barChart",
          "visualizationSettings": {
            "autoSelectVisualization": false
