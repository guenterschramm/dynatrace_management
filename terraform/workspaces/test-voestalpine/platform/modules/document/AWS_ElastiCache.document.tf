  ...
  content   = jsonencode({
      ...
      "tiles": {
        ...
        "17": {
          ...
          "description": "",
          "query": "timeseries bytes = sum(cloud.aws.elasticache.NetworkBytesIn.By.CacheClusterId),\n            by:{CacheClusterId, aws.account.id, aws.region, dt.smartscape_source.id},\n           filter:{in(aws.account.id, array($AccountId)) AND\n                   in(aws.region, array($Region)) AND\n                   in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| fieldsAdd bytesSum = arraySum(bytes)\n| sort bytesSum desc\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "18": {
          ...
          "description": "",
          "query": "timeseries bytes = sum(cloud.aws.elasticache.NetworkBytesOut.By.CacheClusterId),\n            by:{CacheClusterId, aws.account.id, aws.region, dt.smartscape_source.id},\n           filter:{in(aws.account.id, array($AccountId)) AND\n                   in(aws.region, array($Region)) AND\n                   in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| fieldsAdd bytesSum = arraySum(bytes)\n| sort bytesSum desc\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "32": {
          ...
          "description": "",
          "query": "timeseries bytes = sum(cloud.aws.elasticache.BytesWrittenOutFromMemcached.By.CacheClusterId),\n            by:{CacheClusterId, aws.account.id, aws.region, dt.smartscape_source.id},\n           filter:{in(aws.account.id, array($AccountId)) AND\n                   in(aws.region, array($Region)) AND\n                   in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| fieldsAdd bytesSum = arraySum(bytes)\n| sort bytesSum desc\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "33": {
          ...
          "description": "",
          "query": "timeseries bytes = sum(cloud.aws.elasticache.BytesReadIntoMemcached.By.CacheClusterId),\n            by:{CacheClusterId, aws.account.id, aws.region, dt.smartscape_source.id},\n           filter:{in(aws.account.id, array($AccountId)) AND\n                   in(aws.region, array($Region)) AND\n                   in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| fieldsAdd bytesSum = arraySum(bytes)\n| sort bytesSum desc\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
        ...
        "40": {
          ...
          "description": "Efficiency of the cache instance. If the cache ratio is lower than about 0.8, it means that a significant number of keys are evicted, expired, or don't exist.",
          "query": "timeseries {HitsValues = sum(cloud.aws.elasticache.GetHits.By.CacheClusterId),\n            MissesValues = sum(cloud.aws.elasticache.GetMisses.By.CacheClusterId)},\n            by:{CacheClusterId, aws.account.id, aws.region, dt.smartscape_source.id},\n           filter:{in(aws.account.id, array($AccountId)) AND\n                   in(aws.region, array($Region)) AND\n                   in(dt.smartscape_source.id, iCollectArray(toSmartscapeId(array($SmartscapeId)[])))}\n| fieldsAdd rate = HitsValues[] / (HitsValues[] + MissesValues[])\n| fieldsRemove HitsValues, MissesValues\n| sort rate desc\n| limit toLong($Limit)",
          "querySettings": {
            "defaultSamplingRatio": 10,
            "defaultScanLimitGbytes": 500,
            "enableSampling": false,
