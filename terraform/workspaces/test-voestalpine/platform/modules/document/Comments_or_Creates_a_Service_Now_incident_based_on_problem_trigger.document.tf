resource "dynatrace_document" "Comments_or_Creates_a_Service_Now_incident_based_on_problem_trigger" {
  name      = "Comments or Creates a Service Now incident based on problem trigger"
  type      = "workflow-template"
  content   =<<-EOT
    metadata:
      version: '1'
      trackingId: snow-create-update-record
      dependencies:
        apps:
        - id: dynatrace.servicenow
          version: ^2.2.2-dev.20260206T075018+a343a78c
      inputs:
      - type: connection
        ...
        - tasks.create_new_incident.connectionId
    workflow:
      ...
      trigger:
        eventTrigger:
          ...
          filterQuery: event.kind == "DAVIS_PROBLEM" AND event.status == "ACTIVE" AND
            ...
            == "RESOURCE_CONTENTION" OR event.category == "CUSTOM_ALERT" OR event.category
            == "INFO")
      ...
      tasks:
        ...
        create_new_incident:
          ...
          - comment_on_incident
    EOT
  custom_id = "dynatrace.servicenow.comment-create-incident"
  # private = false
}
